...
export async function startLocalEngine(modelPath: string, force = false, ngl?: number, ctxOverride?: number): Promise<LocalStatus> {
  ...
  emitStatus();
  try {
    const prevPid = _proc?.pid;                     // killProc 전에 캡처 — 우리 서버는 절대 죽이지 않게
    await killProc();                               // 기존 서버 종료
    killOrphans(prevPid);                           // 🧟 비정상 종료로 남은 좀비 서버가 포트 1235를 물고 있으면 회수
...
function killOrphans(mine?: number): void {
  try {
    if (process.platform === 'win32') {
      ...
      // ⚠️ 반드시 -sTCP:LISTEN — 안 그러면 1235에 '연결한' 프로세스(우리 Electron 앱 자신!)까지 잡혀 kill될 수 있다.
      const out = execSync(`lsof -ti tcp:${LOCAL_PORT} -sTCP:LISTEN 2>/dev/null || true`, { encoding: 'utf8', timeout: 3000, shell: '/bin/sh' });
