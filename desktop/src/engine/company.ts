...
export interface RunOpts { company: string; agentName?: string; userTitle?: string; workspace?: string; servicesInfo?: string; target?: Partial<LlmTarget>; signal?: AbortSignal; realtimeFor?: (agentId: string) => Promise<string>; getRevenue?: () => Promise<string>; captureScreen?: () => Promise<string | null>; readClipboard?: () => Promise<string>; openPath?: (p: string) => Promise<string>; openApp?: (name: string, url?: string) => Promise<string>; startServer?: (cmd: string) => Promise<string>; attachImages?: string[]; agentModels?: Record<string, string>; getYoutube?: () => Promise<string>; sendTelegram?: (msg: string) => Promise<string>; getGithub?: () => Promise<string>; readRepoFile?: (repo: string, path: string) => Promise<string>; checkEmail?: () => Promise<string>; maxIters?: number; onTerminal?: (kind: 'cmd' | 'out' | 'exit', text: string) => void; }
