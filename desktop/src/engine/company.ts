...
export interface RunOpts { company: string; agentName?: string; userTitle?: string; workspace?: string; servicesInfo?: string; target?: Partial<LlmTarget>; signal?: AbortSignal; realtimeFor?: (agentId: string) => Promise<string>; getRevenue?: () => Promise<string>; captureScreen?: () => Promise<string | null>; readClipboard?: () => Promise<string>; openPath?: (p: string) => Promise<string>; openApp?: (name: string, url?: string) => Promise<string>; startServer?: (cmd: string) => Promise<string>; attachImages?: string[]; agentModels?: Record<string, string>; getYoutube?: () => Promise<string>; sendTelegram?: (msg: string) => Promise<string>; getGithub?: (repo?: string) => Promise<string>; readRepoFile?: (repo: string, path: string) => Promise<string>; checkEmail?: () => Promise<string>; maxIters?: number; onTerminal?: (kind: 'cmd' | 'out' | 'exit', text: string) => void; }
...
async function buildRegistry(opts: RunOpts, workspace: string, target: LlmTarget, onEvent: (e: EngineEvent) => void): Promise<RegEntry[]> {
  ...
  if (opts.getGithub) reg.push({ name: 'get_github', description: '내 깃허브 레포 실데이터(최근 커밋·메시지·날짜). 개발 현황 파악·코드 작업 전에 먼저 이걸로 확인해라. repo에 서비스의 owner/repo를 주면 그 서비스 레포를, 비우면 기본 레포를 본다', event: 'github', parameters: obj({ repo: { type: 'string', description: '조회할 레포 owner/repo (비우면 기본 레포)' } }), run: async a => { const r = await opts.getGithub!(a.repo ? String(a.repo) : undefined); return { text: r, ok: !r.startsWith('('), path: String(a.repo || 'GitHub') }; } });
  ...
  // 💻 서비스 레포 코드 읽기/수정 — 등록 서비스의 깃헙 레포(owner/repo)를 직접 손본다
  if (opts.readRepoFile) reg.push({ name: 'read_repo_file', description: '등록 서비스의 깃허브 레포에서 파일(또는 폴더)을 읽는다. 코드를 고치기 전에 반드시 현재 내용을 먼저 읽어라. path를 빈 폴더 경로로 주면 목록을 본다(예: "" 또는 "docs")', event: 'github', parameters: obj({ repo: { type: 'string', description: '서비스의 레포 owner/repo' }, path: { type: 'string', description: '레포 안 파일 경로 (예: index.html, docs/index.html). 폴더 목록은 폴더 경로' } }, ['repo', 'path']), run: async a => { const r = await opts.readRepoFile!(String(a.repo || ''), String(a.path || '')); return { text: r, ok: !r.startsWith('⚠️'), path: `${a.repo}/${a.path}` }; } });
  reg.push({ name: 'edit_repo_file', description: '등록 서비스의 깃허브 레포 파일을 수정/생성한다(코드·메타·SEO/GEO). 반드시 read_repo_file로 현재 내용을 먼저 본 뒤 전체 새 내용을 넘겨라. 결재 승인 후 실제 푸시됨(GitHub Pages면 즉시 라이브). minor=true는 가벼운 SEO·GEO 수정(메타태그·llms.txt·sitemap·JSON-LD), false는 핵심 코드/내용 변경', event: 'approve', parameters: obj({ repo: { type: 'string', description: '대상 레포 owner/repo' }, path: { type: 'string', description: '레포 안 파일 경로' }, content: { type: 'string', description: '파일의 전체 새 내용(부분 아님)' }, title: { type: 'string', description: '결재에 보여줄 한 줄 요약' }, minor: { type: 'boolean', description: '가벼운 SEO·GEO 수정이면 true' } }, ['repo', 'path', 'content']), run: async a => { const minor = a.minor === true || a.minor === 'true'; const tag = minor ? '🔍 가벼운 SEO·GEO 수정' : '⚠️ 핵심 코드 변경'; addApproval(`${tag}: ${(a.title || a.path).slice(0, 50)}`, `⚡ ${String(a.repo)}/${String(a.path)}`, minor ? '🔍' : '💻', { kind: 'github', repo: String(a.repo || ''), path: String(a.path || ''), payload: a.content || '' } as any); return { text: `${minor ? '가벼운 수정' : '핵심 변경'} 결재를 올렸어요 — 승인하면 ${a.repo} 레포에 실제 반영됩니다.`, ok: true, path: a.path }; } });
