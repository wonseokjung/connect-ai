...
import { fetchUrl, siteMeta, pingUrl } from './engine/web';
...
function openExternalSafe(raw: string): boolean {
  ...
  if (!url) { logDiag(`blocked external URL: ${String(raw || '').slice(0, 160)}`); return false; }
  try {
    void shell.openExternal(url).catch((e: any) => logDiag(`openExternal failed: ${e?.message || e}`));
    return true;
  } catch (e: any) {
    logDiag(`openExternal failed: ${e?.message || e}`);
    return false;
  }
}
function expandHomePath(input: string) {
  return String(input || '').trim().replace(/^~(?=\/|$)/, os.homedir());
}
...
function activeWorkspaceRoot() {
  return path.resolve(expandHomePath(loadConfig().workspace || defaultWorkspace()));
}
function resolveWorkspacePath(input?: string): { ok: true; path: string; root: string } | { ok: false; error: string; root: string } {
  const root = activeWorkspaceRoot();
  const raw = String(input || root).trim();
  if (!raw || raw.length > 4096 || raw.includes('\0')) return { ok: false, root, error: '잘못된 경로입니다.' };
  const expanded = expandHomePath(raw);
  const resolved = path.resolve(path.isAbsolute(expanded) ? expanded : path.join(root, expanded));
  if (!isInsidePath(root, resolved)) return { ok: false, root, error: '작업 폴더 밖의 경로는 작업실에서 직접 열 수 없어요. 먼저 작업 폴더를 변경하세요.' };
  return { ok: true, path: resolved, root };
}
function pathError(error: string, name = '') {
  return { error, name };
}
...
function openRevenueWindow() {
  ...
  revenueWin.webContents.setWindowOpenHandler(({ url }) => { openExternalSafe(url); return { action: 'deny' }; });
...
function openOfficeWindow() {
  ...
  officeWin.webContents.setWindowOpenHandler(({ url }) => { openExternalSafe(url); return { action: 'deny' }; });
...
function openUrlFront(url: string) {
  ...
  try {
    if (process.platform === 'darwin') { spawn('open', [safe], { detached: true, stdio: 'ignore' }).unref(); return true; }
  } catch { /* 폴백 */ }
  return openExternalSafe(safe);
}
...
ipcMain.handle('ops:openArtifact', (_e, rel: string) => {
  try {
    ...
    if (!fs.existsSync(resolved.path)) return { ok: false, reason: '파일을 찾을 수 없어요 (이동·삭제됐을 수 있음)' };
    shell.openPath(resolved.path);
    return { ok: true };
  ...
  return new Promise((resolve) => {
    ...
    child.on('exit', (code) => {
      ...
      if (serveFile) {
        ...
        if (resolved.ok && fs.existsSync(resolved.path)) { shell.openPath(resolved.path); return resolve(`서버 대신 파일을 직접 열었어요: ${resolved.path} (브라우저에 표시됨)`); }
      }
...
function buildRunOpts(c: Config, signal: AbortSignal, attachImages: string[] = []) {
  ...
  const openPath = async (p: string): Promise<string> => {
    ...
    try {
      ...
      if (!resolved.ok) return `열기 실패: ${resolved.error}`;
      if (!fs.existsSync(resolved.path)) return `열기 실패: 그 경로에 파일이 없어요 (${resolved.path})`;
      const err = await shell.openPath(resolved.path);
      return err ? `열기 실패: ${err}` : `✅ 열었어요: ${resolved.path}`;
...
ipcMain.handle('order:list', () => {
  ...
  return orders.map(o => ({ id: o.id, title: o.title, status: o.status, summary: orderSummary(o), createdAt: o.createdAt, currentStage: o.currentStage, sessionRoot: o.sessionRoot, liveUrl: o.liveUrl || '' }));
});
...
ipcMain.handle('fs:tree', (_e, root?: string) => {
  const resolvedRoot = resolveWorkspacePath(root);
  if (!resolvedRoot.ok) return { root: resolvedRoot.root, name: path.basename(resolvedRoot.root), children: [], error: resolvedRoot.error };
  return buildTree(resolvedRoot.path);
});
ipcMain.handle('fs:read', (_e, p: string) => {
  try {
    const resolved = resolveWorkspacePath(p);
    if (!resolved.ok) return pathError(resolved.error);
    const st = fs.statSync(resolved.path);
    const name = path.basename(resolved.path);
    if (/\.(png|jpe?g|gif|webp|bmp|ico|svg)$/i.test(resolved.path)) return { image: `data:image/${(path.extname(resolved.path).slice(1) || 'png').replace('jpg', 'jpeg')};base64,${fs.readFileSync(resolved.path).toString('base64')}`, name };
    if (/\.(pdf|zip|mp4|mov|mp3|wav|dmg|exe|woff2?|ttf|otf|node|bin)$/i.test(resolved.path)) return { binary: true, name };
    if (st.size > 600_000) return { error: `파일이 커서 미리보기를 생략했어요 (${Math.round(st.size / 1024)}KB)`, name };
    return { content: fs.readFileSync(resolved.path, 'utf8'), name, size: st.size };
...
ipcMain.handle('fs:write', (_e, p: string, content: string) => {
  try {
    const resolved = resolveWorkspacePath(p);
...
ipcMain.handle('term:run', (_e, cmd: string, ws?: string) => {
  ...
  if (!resolved.ok) { termSend('out', `${resolved.error}\n`); return false; }
  const st = fs.statSync(resolved.path);
  if (!st.isDirectory()) { termSend('out', `작업 폴더가 아닙니다: ${resolved.path}\n`); return false; }
  termSend('cmd', `$ ${cmd}`);
  spawnInTerminal(normalizeCmd(cmd), resolved.path);
  return true;
});
...
async function runAutoCycleOnce() {
  ...
  try {
    ...
    let sitesCount = 0;
    let sitesUp = 0, sitesDown = 0;   // v0.4.13 — 작업 2: liveUrl ping 결과
    try {
      const orders = listOrders(companyDir).filter((o: any) => o.status === 'completed' || o.status === 'aborted');
      sitesCount = orders.length;
      // v0.4.13 — 작업 2: liveUrl 이 있으면 ping 해서 살아있는지 표시.
      if (orders.length) {
        const pinged = await Promise.all(orders.map(async (o: any) => {
          if (!o.liveUrl) return '- ' + o.title + ' [' + o.status + ', 로컬만]';
          const p = await pingUrl(o.liveUrl).catch(() => ({ ok: false, status: 0, ms: 0 }));
          if (p.ok) sitesUp++; else sitesDown++;
          return '- ' + o.title + ' [' + o.status + ', ' + (p.ok ? '✅ ' + p.status + ' ' + p.ms + 'ms' : '❌ ' + (p.status || '응답없음')) + (o.liveUrl ? ', ' + o.liveUrl : '') + ']';
        }));
        sitesCtx = pinged.join('\n');
      }
    } catch { /* */ }
    ...
    _lastCycleCtx = `사이트 ${sitesCount}개 (정상 ${sitesUp}/비정상 ${sitesDown}), 매출 ${hasRevenue ? '있음' : '없음'}, 노트 참조`;
    ...
    /* v0.4.13 — 작업 1: chat() 단발 → agentWithTools() 전환. 도구 루프가 <write_file>/<run_command> 를
       실행해서 사이클이 site/ 를 직접 수정하고 deploy_cli 로 재배포할 수 있게. */
    const deployCli = path.join(app.getAppPath(), 'assets', 'tool-seeds', 'developer', 'deploy_cli.py');
    // 첫 completed 오더의 site/ 를 workspace 로 (사이클이 고칠 사이트 지정)
    let cycleWs = c.workspace || defaultWorkspace();
    try { const completed = listOrders(companyDir).filter((o: any) => o.status === 'completed'); if (completed.length) { const siteDir = path.join(completed[0].sessionRoot, 'site'); if (fs.existsSync(siteDir)) cycleWs = siteDir; } } catch { /* */ }
    const cycleOpts: any = buildRunOpts(c, new AbortController().signal);
    cycleOpts.workspace = cycleWs;
    cycleOpts.maxIters = 6;
    const cycleSys = sys + '\n\n[도구 사용 안내]\n사이트를 개선하려면 <write_file path="..."> 로 site/ 파일을 직접 수정하세요. 재배포가 필요하면 <run_command>python3 "' + deployCli + '" --dir "' + cycleWs + '"</run_command> 을 실행하세요 (VERCEL_TOKEN 환경변수 필요).';
    const cycleEvents = (e: any) => { try { if (e.kind === 'tool' || e.kind === 'status') win?.webContents.send('engine:event', e); } catch { /* */ } };
    let out = '';
    try {
      out = await agentWithTools([], '현재 시각: ' + new Date().toLocaleString('ko-KR') + '. 위 사이트들의 가치를 높이는 한 걸음을 진행하세요. 도구로 직접 수정하거나 배포할 수 있습니다.', cycleOpts, cycleEvents);
    } catch (e: any) {
      /* agentWithTools 실패 시 chat() fallback (도구 없는 순수 텍스트 사이클) */
      out = await chat(target, cycleSys, '현재 시각: ' + new Date().toLocaleString('ko-KR') + '. 회사 가치를 높이는 한 걸음을 진행하세요.', { temperature: 0.6 }).catch(() => '');
    }
    ...
    writeCycleHealth(companyDir, true, undefined, { sitesCount, hasRevenue, ctx: _lastCycleCtx, sitesUp, sitesDown });
...
function writeCycleHealth(companyDir: string, ok: boolean, reason?: string, meta?: { sitesCount?: number; hasRevenue?: boolean; ctx?: string; sitesUp?: number; sitesDown?: number }) {
  try {
    ...
    if (meta) { if (meta.sitesCount !== undefined) entry.sitesCount = meta.sitesCount; if (meta.hasRevenue !== undefined) entry.hasRevenue = meta.hasRevenue; if (meta.ctx) entry.ctx = meta.ctx; if (meta.sitesUp !== undefined) entry.sitesUp = meta.sitesUp; if (meta.sitesDown !== undefined) entry.sitesDown = meta.sitesDown; }
